				...
				<?php foreach ($users->result() as $m): ?>
					<tr id="tabel<?=$m->ID?>" class="list-item tabel<?=$m->ID?> <?php if (empty($m->Password)) { echo 'table-warning'; }?>">
						...
						<td> <?php if ($m->Status === 'Mahasiswa') { ?>
							...
							<a title="Mengubah Status Mahasiswa?" text="Mahasiswa Akan Mengakses Semua Fitur Untuk Skripsi, Pastikan Semua Persyaratan Telah Terpenuhi Sebelum Mengubah Status" href="<?=base_url('Admin/statusSkripsi/'.$m->ID);?>"  id="<?=$m->ID;?>"> <span class="badge badge-primary"> <?= $m->Status; ?> </span> </a>
